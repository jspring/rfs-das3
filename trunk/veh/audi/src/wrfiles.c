...
void do_buff_inits(int lines_to_save, int do_evt300, int do_audi,
	buff_typ **ppbuff_evt300A, buff_typ **ppbuff_audi)
{
        if (do_evt300) {
                *ppbuff_evt300A = buff_init(f_evt300A, lines_to_save);
        }
        if (do_audi) {
                *ppbuff_audi = buff_init(f_audi, lines_to_save);
        }
}
...
int main(int argc, char *argv[])
{
	...
	bool_typ do_audi = TRUE;
	bool_typ use_memory = FALSE;
	FILE *first_file;
	char *first_file_str;
	char *ini_file = "wrfiles.ini";
	FILE *pfin1 = NULL;
	char *config_file_section = "audi";
	int i;
	 ...
	 */
        ...
        if (use_memory)
                do_buff_inits(lines_to_save, do_evt300, do_audi, 
			&pbuff_evt300A, &pbuff_audi);
	 ...
	 * timestamp" in all of the files. */
		...
		if (do_audi) {
                        save_audi(f_audi, timestamp, use_memory,
                                pbuff_audi);
                }
