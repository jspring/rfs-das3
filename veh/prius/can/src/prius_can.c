...
printf("prius_can: clt_trig_set OK for DB_KOMODO_VAR %d\n", DB_KOMODO_VAR);
	...
	db_clt_write(pclt, DB_PRIUS_MSG343_VAR, sizeof(prius_cruise_control_state_t), &prius_cruise_control_state);
	db_clt_write(pclt, DB_PRIUS_MSG343_VAR, sizeof(prius_vehicle_speed_t), &prius_vehicle_speed);
	db_clt_write(pclt, DB_PRIUS_MSG343_VAR, sizeof(prius_wheel_speed_t), &prius_wheel_speed);
	db_clt_write(pclt, DB_PRIUS_MSG343_VAR, sizeof(prius_long_lat_accel_t), &prius_long_lat_accel);
	...
	for(;;) {
		...
		** If the trigger is from the timer, it's time to send the acceleration message */
		...
		if( DB_TRIG_VAR(&trig_info) == DB_KOMODO_VAR ) {
			...
			switch(db_kom.id) {
				case 0xAA:
					get_prius_wheel_speed(db_kom.msg, &prius_wheel_speed);
					...
					if(verbose)
						print_wheel_speed(&prius_wheel_speed);
					break;
				case 0xB4:
					get_prius_vehicle_speed(db_kom.msg, &prius_vehicle_speed);
					...
					if(verbose)
						print_vehicle_speed(&prius_vehicle_speed);
					break;
				case 0x228:
					get_prius_long_lat_accel(db_kom.msg, &prius_long_lat_accel);
					...
					db_clt_write(pclt,DB_PRIUS_MSG228_VAR, sizeof(prius_long_lat_accel), &prius_long_lat_accel); 
					if(verbose)
						print_long_lat_accel(&prius_long_lat_accel);
					break;
					break;
				case 0x343:
					get_prius_accel_cmd_status(db_kom.msg, &prius_accel_cmd_status);
					check_msg_timeout(ts_ms, &prius_accel_cmd_status.ts_ms, 
						&prius_accel_cmd_status.two_message_periods, 
						&prius_accel_cmd_status.message_timeout_counter); 
					db_clt_write(pclt,DB_PRIUS_MSG343_VAR, sizeof(prius_accel_cmd_status_t), &prius_accel_cmd_status); 
					if(verbose)
						print_accel_cmd_status(&prius_accel_cmd_status);
					break;
				case 0x399:
					get_prius_cruise_control_state(db_kom.msg, &prius_cruise_control_state);
					...
					db_clt_write(pclt,DB_PRIUS_MSG343_VAR, sizeof(prius_cruise_control_state), &prius_cruise_control_state); 
					if(verbose)
						print_cruise_control_state(&prius_cruise_control_state);
					break;
				default:
//					printf("Unknown message %#hx received\n");
			...
			db_clt_read(pclt,DB_PRIUS_MSG99_VAR, sizeof(prius_accel_cmd_t), &prius_accel_cmd); 
			db_clt_write(pclt,DB_PRIUS_MSG99_VAR, sizeof(prius_accel_cmd_t), &prius_accel_cmd); 
			TIMER_WAIT (ptimer);
