...
//#ifdef LT_LINUX
    ...
    sprintf( lPortName, "%s", aPortName );
    *aHandle = open( lPortName, O_RDWR | O_NOCTTY );
//printf("OS.c: OpenSerialPort: LT_LINUX: port %s aHandle %d\n", lPortName, *aHandle);
    ...
    if ( *aHandle >= 0 )
    {
        ...
        {
//printf("OS.c: OpenSerialPort: Got to 1\n");
        	// 8 bits per char, ignore modem control lines, enable receiver.
//#define CS8               0x30
//#define CLOCAL      0x00000800
//#define CREAD       0x00000080
			tio.c_cflag = CS8 | CLOCAL | CREAD;
//printf("OS.c: OpenSerialPort: Got to 2 tio.c_flag %#X\n", tio.c_cflag);
...
//printf("OS.c: OpenSerialPort: Got to 3 LT_SERIAL_SPEED %d tio.c_flag %#X\n", LT_SERIAL_SPEED, tio.c_cflag);
			if ( LT_STOP_BITS == 2 )
			{
				tio.c_cflag |= CSTOPB;
			}
//printf("OS.c: OpenSerialPort: Got to 4 LT_STOP_BITS %d tio.c_flag %#X\n", LT_STOP_BITS, tio.c_cflag);
			switch( LT_PARITY )
			{
			    case LT_PARITY_ODD:
...
//printf("OS.c: OpenSerialPort: Got to 5 LT_PARITY %d tio.c_flag %#X\n", LT_PARITY, tio.c_cflag);
...
//printf("OS.c: OpenSerialPort: Got to 6 tio.c_cflag %#X tio.c_iflag %#X \n", tio.c_cflag, tio.c_iflag);
			if ( !tcsetattr( *aHandle, TCSANOW, &tio ) )
			{
//printf("OS.c: OpenSerialPort: Got to 7\n");
		    	return LT_SUCCESS;
			}
			else
				perror("tsetattr");
