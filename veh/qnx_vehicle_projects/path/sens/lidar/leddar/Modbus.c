...
/*
printf("ModbusSend: message is ");
for(i=0; i<aLength; i++)
	printf("%u:%#hhX ", i, lMessage[i]);
printf("\n");
*/
...
//printf("ModbusConnect: Got to 1 lResult %d\n", lResult);
    if ( lResult == LT_SUCCESS )
    {
        gAddress = aAddress;
//printf("ModbusConnect: Got to 2\n");
       lResult = ModbusSend( MODBUS_SERVER_ID, NULL, 0 );
//        lResult = ModbusSend( 0x41, NULL, 0 );
//printf("ModbusConnect: Got to 3 lResult %d\n", lResult);
...
//printf("ModbusConnect: Got to 4 lResult %d\n", lResult);
...
// *****************************************************************************
// Function: ModbusReadInputRegisters
//
/// \brief   Implements the standard Modbus function 4.
///
/// \param   aFirst   Address of first register to read.
/// \param   aCount   Number of registers to read.
/// \param   aValues  Pointer to array where to write the values (only updated
///                   if LT_SUCCESS is returned).
///
/// \return  LT_SUCCESS or any one of the LT error codes.
// *****************************************************************************
